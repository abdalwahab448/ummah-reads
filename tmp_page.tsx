        ...
        {showVideo ? (
          ...
          >
            <video
              ...
              onEnded={() => setStep(2)}
              onError={(event) => {
                console.error("Video failed to load, switching to the overview view:", event);
                setStep(2);
              }}
            ...
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(5,10,8,0.25),rgba(5,10,8,0.9))]" />

            <button
              type="button"
              onClick={() => setStep(2)}
              className="absolute left-4 top-4 z-20 rounded-full border border-[#d4af37]/40 bg-[#16382b]/80 px-5 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[#d4af37] backdrop-blur-md transition hover:bg-[#d4af37] hover:text-[#0d231a]"
            >
              تخطي العرض ←
            </button>
          </motion.section>
